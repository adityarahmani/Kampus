...
public class MahasiswaViewFrame extends JFrame{
    ...
    public MahasiswaViewFrame() {
        ...
        cariButton.addActionListener(e -> {
            ...
            String searchSQL = "SELECT * FROM mahasiswa WHERE nama like ?";
            try {
        ...
        hapusButton.addActionListener(e -> {
            ...
            if (pilihan == 0 ) {
                ...
                Connection c = Koneksi.getConnection();
                String deleteSQL = "DELETE FROM mahasiswa WHERE id = ?";
                try {
        ...
        tambahButton.addActionListener(e -> {
            MahasiswaInputFrame inputFrame = new MahasiswaInputFrame();
        ...
        cetakButton.addActionListener(e -> {
            ...
            try {
                ...
                row = new Object[jumlah][7];
                int i = 0;
                rs.beforeFirst();
                while (rs.next()){
                    row[i][0] = rs.getInt("id");
                    row[i][1] = rs.getString("nim");
                    row[i][2] = rs.getString("nama");
                    row[i][3] = rs.getString("alamat");
                    row[i][4] = rs.getString("jenis_kelamin");
                    row[i][5] = rs.getString("email");
                    row[i][6] = rs.getString("telepon");
                    i++;
                }
            } catch (SQLException ex) {
                throw new RuntimeException(ex);
            ...
            try {
                JasperReport jasperReport =
                        JasperCompileManager.compileReport("C:\\Users\\adiya\\IdeaProjects\\kampus\\src\\main\\resources\\mahasiswa_report.jrxml");
    ...
    public void isiTable(){
        ...
        try {
            ...
            String[] header = {"id","NIM","Nama","Alamat","Jenis Kelamin","email","telepon"};
